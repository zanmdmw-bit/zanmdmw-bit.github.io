...
function snapToAnchor() {
  const part = state.selected;
  const anchor = blueprint.anchors.find((item) => item.id === dom.anchorSelect.value);
...
async function importAsset(event) {
  ...
  try {
    ...
    state.model.registerExternal(object, stripExtension(file.name));
    state.history.push(before);
    trimHistory();
    state.future = [];
    markDirty();
...
function populateAnchors() {
  dom.anchorSelect.innerHTML = blueprint.anchors.map((anchor) => `<option value="${anchor.id}">${escapeHtml(anchor.name)}</option>`).join("");
}
